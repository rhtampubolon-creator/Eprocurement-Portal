...
const isAdmin=role==="PROCUREMENT_ADMIN"||role==="SUPER_ADMIN";
const canView=isBuyer||isAdmin||isUser;
...
window.updateRequest=async function(id){if(isUser){toast("Akun USER hanya dapat melihat data");return;}try{const status=document.querySelector(`[data-status="${CSS.escape(id)}"]`).value,adminNote=document.querySelector(`[data-note="${CSS.escape(id)}"]`).value;await api({action:"UPDATE_VENDOR_REQUEST",requestId:id,status,adminNote});toast("Status berhasil diperbarui");loadRequests()}catch(e){toast(e.message)}};
document.getElementById("vendorRequestForm").addEventListener("submit",async e=>{e.preventDefault();if(isUser){toast("Akun USER hanya dapat melihat data");return;}try{const payload=Object.fromEntries(new FormData(e.currentTarget));payload.action="CREATE_VENDOR_REQUEST";await api(payload);e.currentTarget.reset();toast("Permintaan berhasil dikirim");loadRequests()}catch(err){toast(err.message)}});
document.getElementById("refreshBtn").addEventListener("click",loadRequests);
document.addEventListener("DOMContentLoaded",()=>{if(!canView){toast("Role Anda tidak memiliki akses");return}document.getElementById("buyerFormCard").classList.toggle("hidden",!isBuyer);document.getElementById("listTitle").textContent=isBuyer?"My Vendor Requests":(isUser?"Vendor Requests":"Vendor Request Approval");document.getElementById("roleInfo").textContent=isBuyer?"Ajukan dan pantau permintaan vendor":(isUser?"View seluruh vendor request (read only)":"Review dan proses permintaan dari Buyer");document.getElementById("adminActionHead").style.display=isAdmin?"":"none";loadRequests()});
