/* ======================================================
   ...
   Release 3.5.3
   - Restores inline Action handlers without changing the stable module script.
   - Retries revision-protected batch mutations once with a fresh server revision.
   - Keeps USER/read-only permissions enforced through the existing MSW auth layer.
====================================================== */
(function installProcurementActionBridge(){
  'use strict';
  if (window.__MSW_PROCUREMENT_ACTION_BRIDGE_V353__) return;
  window.__MSW_PROCUREMENT_ACTION_BRIDGE_V353__ = true;

  const MODULE_ID = 'procurementAdmin';
  const SHEET_NAME = 'Admin';
  const CACHE_KEY = 'MSW_PROCUREMENT_CACHE';
  const PROTECTED_BATCH_ACTIONS = new Set([
    'BATCH_IMPORT_PROCUREMENT_BY_BUYER',
    'BATCH_REPLACE_PROCUREMENT'
  ]);

  function gasUrl(){
    return String(window.APP_CONFIG?.GAS_URL || '').trim();
  }

  function isViewOnly(){
    try { return Boolean(window.MSW?.auth?.isViewOnlyModule?.(MODULE_ID)); }
    catch (_) { return false; }
  }

  function blockMutation(){
    if (!isViewOnly()) return false;
    try { window.MSW?.auth?.showViewOnlyMessage?.(); } catch (_) {}
    return true;
  }
  ...
  function isRevisionConflict(payload){
    ...
    return Boolean(payload.conflict) ||
      message.includes('berubah sejak halaman dibuka') ||
      message.includes('revision conflict') ||
      message.includes('revision berubah') ||
      (message.includes('revision') && message.includes('muat ulang'));
  }
  ...
  async function fetchFreshAdminSnapshot(){
    ...
    // Query unik mencegah stale-while-revalidate mengembalikan snapshot lama.
  ...
  // Lapisan kompatibilitas multi-user. Proteksi revision TIDAK dihapus.
  // Jika user lain menyimpan lebih dulu, ambil revision server terbaru lalu retry sekali.
  window.fetch = async function procurementRevisionAwareFetch(input, init){
    ...
    if (!gas || method !== 'POST' || url.indexOf(gas) !== 0) {
      return upstreamFetch(input, init);
    }

    const payload = parseJsonBody(options.body);
    const action = String(payload?.action || '').trim().toUpperCase();
    if (!payload || !PROTECTED_BATCH_ACTIONS.has(action) || payload.expectedRevision == null) {
      return upstreamFetch(input, init);
    }
    ...
    try {
      ...
      notify('Data berubah oleh user lain. Revision terbaru dimuat dan proses dicoba kembali.', 'info');
      const retryPayload = Object.assign({}, payload, {
        expectedRevision: latestRevision,
        clientRetryAfterRevisionConflict: true
      });
      ...
      console.warn('Multi-user retry tidak dapat dijalankan:', error);
  ...
  function openProcurementForm(url){
    ...
    if (!frame || !modal || !formWindow) {
      notify('Procurement Form belum siap. Muat ulang halaman lalu coba kembali.', 'error');
      return;
    }
  ...
  window.handleAdd = function handleAddBridge(){
    openProcurementForm('Form/index.html?mode=add');
  };

  function cacheRows(){
    try {
      const rows = window.MSW?.cache?.load?.(CACHE_KEY);
  ...
  window.exportExcel = function exportExcelBridge(){
    const rows = cacheRows();
    if (!rows.length) {
      notify('Tidak ada data untuk diekspor.', 'info');
      return;
    }
    if (!window.XLSX) {
      notify('Library Excel belum termuat. Klik Export Excel sekali lagi.', 'error');
      return;
    }
    ...
    const exportRows = rows.map(r => {
      ...
      roundList.forEach(round => {
        ...
        out[`${round} Company`] = r[`${key}company`] || '';
        out[`${round} Submit Company`] = r[`${key}submitcompany`] || '';
        out[`${round} Start Date`] = r[`${key}startdate`] || '';
        out[`${round} Finish Date`] = r[`${key}finishdate`] || '';
      });

      Object.assign(out, {
        'Winner PO': r.winnerpo || '',
        'Email Winner PO': r.emailwinnerpo || '',
        'No PO': r.nopo || '',
        'Price (Rp) Excl. PPn': r.pricerp || '',
        'CQS Create Date': r.cqscreatedate || '',
        'CQS Approval Date': r.cqsapprovaldate || '',
        'PO Create Date': r.pocreatedate || '',
        'PO Del. Date': r.podeldate || '',
        'Actual PO Rel. Date': r.actualporeleasedate || '',
        'Actual PO Del. Date': r.actualpodeldate || '',
        'Days Calender (Days)': r.days || '',
        'Actual Received PO (GRN Date)': r.actualreceivedpo || '',
        'Note': r.note || '',
        'Folder ID': r.folderid || '',
        'Folder LINK': r.folderlink || ''
      });
      return out;
    });
    ...
    XLSX.writeFile(wb, `Procurement Admin-${new Date().toISOString().slice(0,10)}.xlsx`);
  };
  ...
  window.clearAll = async function clearAllBridge(){
    if (blockMutation()) return;
    ...
    try {
      ...
      const gas = gasUrl();
      const response = await window.fetch(gas, {
        method: 'POST',
      ...
      if (!response.ok || result?.success === false) {
        throw new Error(result?.message || `Google Sheet gagal diperbarui (HTTP ${response.status}).`);
      }

      if (result?.queued || result?.pendingSync) {
        notify('All Clear tersimpan sebagai Pending Sync dan akan dijalankan saat koneksi tersedia.', 'info');
        return;
      }
      ...
      notify(`All Clear gagal: ${error?.message || error}`, 'error');
      try { await fetchFreshAdminSnapshot(); } catch (_) {}
    }
  };
  ...
  function formatDate(value){
    ...
    return date ? date.toLocaleDateString('en-GB', {day:'2-digit',month:'short',year:'numeric'}) : String(value || '');
  }

  // Round PR adalah kontrol tampilan saja. Bridge ini mencegah inline onchange
  // gagal ketika fungsi modul berada di block scope.
  window.updateRoundPR = function updateRoundPRBridge(index, selectedValue){
    const rows = cacheRows();
    ...
    const company = row[`${key}company`] || (active ? row.roundcompany : '') || '';
    const submitted = row[`${key}submitcompany`] || (active ? row.roundsubmitcompany : '') || '';
    const start = row[`${key}startdate`] || (active ? row.roundstartdate : '') || '';
    const finish = row[`${key}finishdate`] || (active ? row.roundfinishdate : '') || '';

    const setText = (field, value) => {
      const cell = tr.querySelector(`td[data-key="${field}"]`);
      if (cell) cell.textContent = String(value ?? '');
    };
    setText('roundcompany', company);
    setText('roundsubmitcompany', submitted);
    setText('roundstartdate', formatDate(start));
    setText('roundfinishdate', formatDate(finish));
    setText('finalvendorlist', company && start && finish ? company : '');
    setText('finalsubmitvendor', submitted);
  };
